...
from vp.config import get_config, DIRS, PLATFORM_ROOT, engine_cmd, db_path
...
def _webapp_dir():
    """webapp 目录：源码运行=app.py 同级或上级的 webapp；PyInstaller 打包=内置资源。"""
    here = os.path.dirname(os.path.abspath(__file__))
    for src in (os.path.join(here, 'webapp'),                    # app.py 同级（本仓库布局）
                os.path.join(os.path.dirname(here), 'webapp')):  # vp 包布局
        if os.path.isdir(src):
            return src
    if getattr(sys, 'frozen', False):                          # 打包资源
        base = getattr(sys, '_MEIPASS', os.path.dirname(os.path.abspath(sys.executable)))
        bundled = os.path.join(base, 'webapp')
        if os.path.isdir(bundled):
            return bundled
    return os.path.join(here, 'webapp')
...
# ------------------------------------------------------------------
# Open-Virome 公共病毒组模块（平台壳页 + 同源 iframe 内嵌 SPA，远程 API）
# ------------------------------------------------------------------
_VIROME_DIST = os.path.join(_WWW, 'static', 'virome')
# 源码运行时优先直接用前端 build 产物（重新构建即时生效，无需拷贝）；
# 打包部署（PyInstaller）则回退到 webapp/static/virome 副本。
...
@app.route('/virome/<path:filename>')
...
# 存储与磁盘水位
# ------------------------------------------------------------------
# 磁盘剩余用 shutil.disk_usage，毫秒级、每次请求实时算；
# 目录占用要遍历 6 万+ 文件（databases 47GB），实测数十秒，因此走
# 后台线程 + 缓存，绝不能放在请求路径上同步扫描。
...
def _scan_targets():
    ...
    items = [
        ...
        ('tool_runs', _tool_runs_root(), 'output'),
